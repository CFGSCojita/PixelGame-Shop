<?php
    ...
    include($root_DIR . '/student006/shop/backend/api/api_send_orders_Bruno.php'); // Incluimos la función de envío a Bruno
    ...
    // Obtenemos todos los productos del carrito del usuario (incluimos supplier_id para detectar productos de Bruno).
    $sql_carrito = "SELECT 
                    c.videogame_id,
                    c.quantity,
                    v.price,
    ...
    $items_bruno = [];   // Array para acumular los items de Bruno y enviárselos después
    ...
    while ($item = mysqli_fetch_assoc($result_carrito)) {
        ...
        // Si el producto es de Bruno (supplier_id = 2), lo guardamos para enviárselo después
        if ($item['supplier_id'] == 2) $items_bruno[] = $item;
    ...
    if ($pedidos_exitosos > 0) {
        ...
        // Si había productos de Bruno, enviamos el pedido a su API
        if (!empty($items_bruno)) {
            enviar_pedidos_bruno($items_bruno, $user_email, $user_address, $conn);
        }
